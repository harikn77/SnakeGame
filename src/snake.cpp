...
void Snake::Update() {
    ...
    // if (isObstacle(prev_cell.x, prev_cell.y)) alive = false;
    UpdateHead();
...
void Snake::UpdateBody(SDL_Point &current_head_cell, SDL_Point &prev_head_cell) {
    ...
    for (auto const &item : body) {
        if (current_head_cell.x == item.x && current_head_cell.y == item.y || isObstacle(current_head_cell.x, current_head_cell.y)) {
            alive = false;
        }
    }
}
